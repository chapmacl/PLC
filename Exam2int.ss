...
(define-datatype expression expression?
  	...
  	[letrec-exp (ids (list-of symbol?)) (var-bodies (list-of expression?)) (bodies (list-of expression?))]
        ...
        [do1-exp (bodies (list-of expression?)) (condition expression?)]
        [do2-exp (bodies (list-of expression?)) (condition expression?)]
        [cwv-exp (op expression?) (stuff (list-of expression?))]
        [cwv2-exp (op expression?) (stuff expression?)]
		...
		(cond
		    ...
		    [(pair? datum) 
		    	...
		    	[(eqv? (car datum) 'quote) (list 'lit-exp (cadr datum))]
                    ...
                    [(eqv? (car datum) 'do1)
                    	(do1-exp (map parse-exp (cadr datum)) (parse-exp (cadddr datum)))]
                    [(eqv? (car datum) 'do2)
                    	(do2-exp (map parse-exp (cadr datum)) (parse-exp (cadddr datum)))]
                    [(eqv? (car datum) 'call-with-values)
                     (cond
                       [(symbol? (cadr datum)) (cwv2-exp (parse-exp (caddr datum)) (parse-exp '()))]
                       [else (cwv-exp (parse-exp (caddr datum)) (map parse-exp (cdr (caddr (cadr datum)))))]
                       )]
	...
	(lambda (exp)
	    (cases expression exp
		    ...
		    [begin-exp (bodies) (cons* 'begin (map unparse-exp bodies))]
                    ...
                    [while-exp (condition bodies) (cons* 'while (unparse-exp condition) (map unparse-exp bodies))]
            ...
            [case-exp (id keys bodies)
 		    ...
 		    [while-exp (condition bodies)
 		    	(while-exp (syntax-expand condition) (map syntax-expand bodies))]
                    ...
                    [do1-exp (bodies condition) 
                      (begin 
                        (map syntax-expand bodies)
                        (while-exp (syntax-expand condition) (map syntax-expand bodies)))]
                    [do2-exp (bodies condition) 
                      (do2-exp (map syntax-expand bodies) (syntax-expand condition))]
                    [cwv-exp (op stuff)
                      (cwv-exp op stuff)]
                    [cwv2-exp (op stuff)
                      (cwv2-exp op stuff)]
                ...
                [do2-exp (bodies condition)
                  (begin
                    ...
                    (if (eval-exp condition env)
                        (eval-exp exp env)))]
                [cwv-exp (op stuff)
                  (apply (eval-exp op env) (eval-exp stuff))]
                [cwv2-exp (op stuff)
                  (apply (eval-exp op env) '())]
