...
(define *prim-proc-names* '(+ - * / = <= >= < > add1 sub1 zero? not cons car cdr list null? assq eq? eqv? equal? atom? length list->vector list? 
                            ...
                            map apply member? quotient list-tail append))
        ...
        [or-exp (bodies)
            (let loop ([bodies bodies])
                (if (eq? (length bodies) 0)
                    (parse-exp '#f)
                    (let-exp (list 'val) (list (car bodies)) (list (if-then-else-exp (syntax-expand (var-exp 'val)) (syntax-expand (var-exp 'val)) (loop (cdr bodies)))))))]
        [and-exp (bodies)
            (let loop ([bodies bodies])
                (if (eq? (length bodies) 0)
                    (parse-exp '#t)
		...
		(lambda (exp env)
			(cases expression exp
	    		...
	    		[lambda-exp-var (id bodies) 
	    			(closure (map (lambda (x) (if (and (list? x) (eq? (length x) 2) (eqv? (car x) 'ref) (symbol? (cadr x)))
	            ...
	            [letrec-exp (proc-names bodies letrec-body)
	            	(eval-exp letrec-body (extend-env-recursively proc-names bodies env))]
            	...
            	[define-exp (id r-val-exp) (set! global-env (extend-env (list id) (list (eval-exp r-val-exp env)) global-env))]
...
(define apply-proc
	(lambda (proc-value args)
		...
		(cases proc-val proc-value
            ...
            [closure-ref (params bodies env) (eval-bodies bodies (extend-env-ref params args env))]
